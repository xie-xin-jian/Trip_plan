...
class TripRequest(BaseModel):
    ...
    departure_city: str = Field(default="", description="出发城市", example="上海")
    ...
    preferred_transport_type: str = Field(default="all", description="往返交通偏好: all/flight/high_speed_rail/train", example="high_speed_rail")
...
class TransportationOption(BaseModel):
    """单条交通选项"""
    transport_type: str = Field(default="", description="交通类型: flight/train/high_speed_rail/bus")
    transport_name: str = Field(default="", description="车次/航班号，如 G123 / CA1234")
    departure_time: str = Field(default="", description="出发时间 HH:MM")
    arrival_time: str = Field(default="", description="到达时间 HH:MM")
    duration: str = Field(default="", description="耗时，如 4h30m")
    departure_station: str = Field(default="", description="出发站/机场")
    arrival_station: str = Field(default="", description="到达站/机场")
    price_economy: int = Field(default=0, description="经济舱/二等座票价(元)")
    price_business: Optional[int] = Field(default=None, description="商务舱/一等座票价(元)")
    seats_available: str = Field(default="有票", description="余票情况")


class RoundTripTransportation(BaseModel):
    """往返交通信息"""
    departure_city: str = Field(default="", description="出发城市")
    destination_city: str = Field(default="", description="目的地城市")
    outbound: List[TransportationOption] = Field(default=[], description="去程交通选项")
    return_trip: List[TransportationOption] = Field(default=[], description="返程交通选项")
    total_transport_budget: int = Field(default=0, description="往返交通预估总费用(元)")
...
class TripPlan(BaseModel):
    ...
    round_trip_transportation: Optional[RoundTripTransportation] = Field(default=None, description="往返交通信息")
