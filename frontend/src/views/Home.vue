<template>
  <div class="home-container">
    ...
    <div class="top-nav">
      <div class="nav-left">
        <span class="nav-logo">🌍 HelloAgents智能旅行助手</span>
      </div>
      <div class="nav-right">
        <template v-if="currentUser">
          <router-link to="/my-plans" class="nav-link">
            📋 我的行程
          </router-link>
          <span class="nav-user">
            👤 {{ currentUser.username }}
          </span>
          <a-button type="text" class="nav-link-btn" @click="handleLogout">
            退出
          </a-button>
        </template>
        <template v-else>
      ...
      >
        ...
        <div class="form-section">
          ...
          <a-row :gutter="24">
            ...
            <a-col :span="8">
              <a-form-item name="preferred_transport_type" v-if="formData.departure_city">
                <template #label>
                  <span class="form-label">往返交通偏好</span>
                </template>
                <a-select v-model:value="formData.preferred_transport_type" size="large" class="custom-select">
                  <a-select-option value="all">🔀 全部类型</a-select-option>
                  <a-select-option value="high_speed_rail">🚄 高铁/动车</a-select-option>
                  <a-select-option value="flight">✈️ 飞机</a-select-option>
                  <a-select-option value="train">🚂 普通火车</a-select-option>
                </a-select>
              </a-form-item>
            </a-col>
            <a-col :span="8">
...
import { generateTripPlanLangGraph, getCurrentUserInfo, logout, type User } from '@/services/api'
...
interface FormData extends Omit<TripFormData, 'start_date' | 'end_date'> {
...
const handleSubmit = async () => {
  ...
  try {
    const requestData: TripFormData = {
      ...
      preferred_transport_type: formData.preferred_transport_type || 'all',
    ...
    const response = await generateTripPlanLangGraph(requestData)
